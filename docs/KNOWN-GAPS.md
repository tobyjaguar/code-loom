...
(`AGENTS_DIR="$ROOT/.agents"`); the guard's own reads are `bin/loom:2847`
...
**Where.** `bin/loom:4393`, in `cmd_plan` (`bin/loom:4366`):
...
**Where.** `bin/loom:1895`, in `run_headless` (`bin/loom:1808`), the opencode leg:
...
`run_headless`'s opencode leg (`bin/loom:1926`) sets the **disable
...
`config_summary`, `config_programs` at `bin/loom:1250`,
`require_recorded_config`); `bin/loom:1386` onward (the REPOSITORY baseline:
`repo_config_file`, `repo_config_read`, `repo_config_write`,
`repo_config_repin`, and `require_recorded_config_repo` at `bin/loom:1449`),
read from `scout_root` (`bin/loom:4446`) and `cmd_plan`, written by
`cmd_pin_config` (`bin/loom:4262`); `scout_mirror_config_check`
(`bin/loom:4420`) for the one scope neither pin can cover; and the
